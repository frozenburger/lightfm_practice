...
class model():
    def __init__(self, interactions, user_feat_indicators, item_feat_indicators, k_embeddings=30):
        ...
        self.user_feat_embeddings = tf.Variable(self.rng.rand(self.size_uf, self.k_embeddings), dtype=tf.float32) #학습시킬 각 유저피쳐 임베딩
        self.item_feat_embeddings = tf.Variable(self.rng.rand(self.size_if, self.k_embeddings), dtype=tf.float32) #학습시킬 각 상품피쳐 임베딩
        self.user_bias_vector = tf.Variable(self.rng.rand(self.size_u), dtype=tf.float32) # ??
        self.item_bias_vector = tf.Variable(self.rng.rand(self.size_i), dtype=tf.float32) # ??
    ...
    def train(self, epoch=30):
        ...
        for i in range(epoch):
            opt.minimize(self.objective_function, var_list=[self.user_feat_embeddings, self.item_feat_embeddings, self.user_bias_vector, self.item_bias_vector])
