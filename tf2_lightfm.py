...
class model():
    def __init__(self, interactions, user_feat_indicators, item_feat_indicators, labels, test_data, k_embeddings=10):
        ...
        self.opt = tf.keras.optimizers.Adagrad(learning_rate=0.05)
    ...
    def linear_loss(self):
        ...
        return tf.sqrt(tf.reduce_mean(squared_diff))
    ...
    def train_manually(self, epoch=30):
        opt = self.opt
        for i in range(epoch):
            with tf.GradientTape() as tape:
                def loss():
                    ...
                    squared_diff = tf.math.squared_difference(self.true_matrix, positive_scores + negative_scores)
                    return tf.sqrt(tf.reduce_mean(squared_diff))
                gradients = tape.gradient(loss(), [self.user_feat_embeddings, self.item_feat_embeddings, self.user_feat_bias_vector, self.item_feat_bias_vector])
                opt.apply_gradients(zip(gradients, [self.user_feat_embeddings, self.item_feat_embeddings, self.user_feat_bias_vector, self.item_feat_bias_vector]))
                print('loss : {}, learning rate : {}'.format(loss().numpy(), opt.get_config()['learning_rate']))
...
d1, d2, d3, d4, d5 = movielens_split()
cls = model(d1, d2, d3, d4, d5)
cls.train_manually(1)
'''